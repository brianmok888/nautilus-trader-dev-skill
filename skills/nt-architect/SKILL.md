...
| --- | --- | --- | --- |
| G0 Upstream baseline | Pin the developer-guide snapshot and APIs before design. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G1 Legacy label | Keep migration/reference labels on legacy/Cython/v1 guidance outside production design. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
...
| G3 Rust bindings/PyO3 | Name crate ownership, binding registration, and callback routing. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G4 Lane and API shape | Quarantine non-AI Python and keep execution authority in Rust. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G5 Test evidence | Map each component to unit, integration, and lifecycle evidence. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G6 Safety/compliance | Specify fail-closed risk, precision, async, FFI, and audit boundaries. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G7 Completion report | Report changed paths, commands, evidence, and unresolved gates. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
