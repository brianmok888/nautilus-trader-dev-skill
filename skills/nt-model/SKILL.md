...
## Order metadata validation (develop-line)

Upstream commit
[`d46f56505`](https://github.com/nautechsystems/nautilus_trader/commit/d46f56505),
included in the pinned G2 baseline, adds `OrderInitialized::new_checked`. The fallible constructor rejects a
...
`1602043debb82b34084d35a452c374b744b96524`. Use `new_checked` when code must
...
| G2 Pinned V2 examples | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-model` passed the skill domain's scoped examples and owners against `1602043debb82b34084d35a452c374b744b96524`; schema-v2 provenance is recorded in `references/g2-evidence/nt-model.json`. |
...
Use `references/developer_guide/rust.md` and the model snapshots under `references/api/model/` as source-pinned upstream material at commit `1602043debb82b34084d35a452c374b744b96524`. Version-scope post-pin APIs such as `OrderInitialized::new_checked` until the baseline advances.
