...
`8e51f957c6e31b28de14fbe244b3c048e291ddd7`. Use `new_checked` when code must
...
| G2 Pinned V2 examples | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-model` passed the skill domain's scoped examples and owners against `8e51f957c6e31b28de14fbe244b3c048e291ddd7`; schema-v2 provenance is recorded in `references/g2-evidence/nt-model.json`. |
...
Use `references/developer_guide/rust.md` and the model snapshots under `references/api/model/` as source-pinned upstream material at commit `8e51f957c6e31b28de14fbe244b3c048e291ddd7`. Version-scope post-pin APIs such as `OrderInitialized::new_checked` until the baseline advances.
