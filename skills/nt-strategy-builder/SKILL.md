...
| G2 V2 example validation | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Blocked | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-strategy-builder` passed the repository migration tests but is currently blocked before pinned-upstream tests because `python/.venv/bin/python` is unavailable; run `make sync-v2` in the pinned upstream checkout, then rerun the harness. The previous pass remains in `references/g2-evidence/nt-strategy-builder.json` as historical evidence only. |
