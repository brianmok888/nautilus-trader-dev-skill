...
`docs/integrations/betfair.md` in the read-only pinned snapshot (pin `5e4be2edbf`) is itself the
...
- Upstream reference: pinned `docs/integrations/betfair.md` (current v2 Rust-adapter guide at pin `5e4be2edbf`); repo-local entry point in [betfair.md](betfair.md)
...
| Reconciliation scope     | `reconcile_market_ids_only` uses `reconcile_market_ids`; otherwise falls back to `stream_market_ids_filter`. | The pinned upstream doc at `5e4be2edbf` documents the same coupling.          | Resolved - keep current.                             |
...
session logic above and is included in the pinned baseline `5e4be2edbf` (upstream commit
...
(seconds) reflect upstream commit `74d57e7e05`, included in the pinned baseline `5e4be2edbf`;
