...
`docs/integrations/betfair.md` in the read-only pinned snapshot (pin `9bafb63e7d`) is itself the
...
- Upstream reference: pinned `docs/integrations/betfair.md` (current v2 Rust-adapter guide at pin `9bafb63e7d`); repo-local entry point in [betfair.md](betfair.md)
...
| Reconciliation scope     | `reconcile_market_ids_only` uses `reconcile_market_ids`; otherwise falls back to `stream_market_ids_filter`. | The pinned upstream doc at `9bafb63e7d` documents the same coupling.          | Resolved - keep current.                             |
...
session logic above and is included in the pinned baseline `9bafb63e7d` (upstream commit
...
(seconds) reflect upstream commit `74d57e7e05`, included in the pinned baseline `9bafb63e7d`;
