...
`docs/integrations/betfair.md` in the read-only pinned snapshot (pin `1602043d`) is itself the
...
- Upstream reference: pinned `docs/integrations/betfair.md` (current v2 Rust-adapter guide at pin `1602043d`); repo-local entry point in [betfair.md](betfair.md)
...
| Reconciliation scope     | `reconcile_market_ids_only` uses `reconcile_market_ids`; otherwise falls back to `stream_market_ids_filter`. | The pinned upstream doc at `1602043d` documents the same coupling.          | Resolved - keep current.                             |
...
session logic above and is included in the pinned baseline `1602043d` (upstream commit
...
(seconds) reflect upstream commit `74d57e7e05`, included in the pinned baseline `1602043d`;
