...
`docs/integrations/betfair.md` in the read-only pinned snapshot (pin `c1a2310144`) is itself the
...
- Upstream reference: pinned `docs/integrations/betfair.md` (current v2 Rust-adapter guide at pin `c1a2310144`); repo-local entry point in [betfair.md](betfair.md)
...
| Reconciliation scope     | `reconcile_market_ids_only` uses `reconcile_market_ids`; otherwise falls back to `stream_market_ids_filter`. | The pinned upstream doc at `c1a2310144` documents the same coupling.          | Resolved - keep current.                             |
...
session logic above and is included in the pinned baseline `c1a2310144` (upstream commit
...
(seconds) reflect upstream commit `74d57e7e05`, included in the pinned baseline `c1a2310144`;
...
Register the Betfair clients on a Rust `LiveNode` (pinned example:
`crates/adapters/betfair/examples/node_data_tester.rs`):

```rust
use nautilus_betfair::{config::BetfairDataClientConfig, factories::BetfairDataClientFactory};
...
let mut node = LiveNode::builder(TraderId::from("BETFAIR-001"), Environment::Live)?
    .with_name("BETFAIR-DATA")
    .add_data_client(None, Box::new(BetfairDataClientFactory::new()), Box::new(data_config))?
    .build()?;
```
