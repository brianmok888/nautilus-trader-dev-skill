...
| --- | --- | --- | --- |
| G0 Upstream baseline | API claims match the pinned developer-guide snapshot. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G1 Legacy label | NT v2 compatibility note: legacy/Cython/v1 guidance is migration/reference-only and does not enter production code. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G2 V2 example validation | Changed Rust examples compile or have scoped harness evidence. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-review` passed against `6e59fd74eaacacbb7410936f1766bd89fcce6f59`; evidence: `references/g2-evidence/nt-review.json`. |
| G3 Rust bindings/PyO3 | Binding registration, ownership, callback routing, and cleanup are reviewed. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G4 Lane and API shape | Non-AI Python is quarantined; Rust retains execution authority. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G5 Test evidence | Targeted tests plus relevant lint/build commands are recorded. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G6 Safety/compliance | Risk, precision, overflow, secrets, async, FFI, and audit checks pass. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |
| G7 Completion report | Findings cite paths, severity, commands, and remaining risk. | Pass | `uv run python tools/check_skill_g2_harnesses.py --check-cards` records the scoped readiness evidence. |

NT v2 compatibility note: Rust-oriented v2.0 readiness rejects unlabelled legacy/Cython/v1 guidance as migration/reference-only and requires Rust-owned production paths. AI/advisory lane remains Python and off execution-critical paths. Python live `TradingNode` material is reference-only; current Rust-backed live work uses `LiveNode`.
...
Also reject Python v2 config stub/readback drift, stale Generated Python artifacts, and missing `make py-stubs-v2` evidence when binding surfaces change.
...
The only active Python lane is AI/advisory integration. Review it for asynchronous isolation, explicit operator approval, provenance, safe payload scope, timeouts, and deterministic Rust-owned fallback.
...
Validate review claims against the immutable snapshot under [`references/developer_guide/`](../../references/developer_guide/), particularly `rust.md`, `ffi.md`, `testing.md`, `benchmarking.md`, `adapters.md`, and `coding_standards.md`, pinned to commit `6e59fd74eaacacbb7410936f1766bd89fcce6f59`. Version-scope any newer upstream guidance until the pin advances.
