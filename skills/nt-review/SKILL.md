...
| --- | --- | --- | --- |
| G0 Upstream baseline | API claims match the pinned developer-guide snapshot. | Pass | `uv run python tools/check_dev_guide_snapshot_sync.py` verifies the immutable developer-guide snapshot. |
| G1 Legacy label | NT v2 compatibility note: legacy/Cython/v1 guidance is migration/reference-only and does not enter production code. | Pass | `uv run python tools/check_dev_guide_sync.py` enforces migration labels for legacy/Cython/v1 guidance. |
...
| G3 Rust bindings/PyO3 | Binding registration, ownership, callback routing, and cleanup are reviewed. | Pass | `uv run pytest -q tests/test_v2_guidance_hardening.py -k 'pyo3 or binding or rust or live_runner'` validates binding and callback boundaries. |
| G4 Lane and API shape | Non-AI Python is quarantined; Rust retains execution authority. | Pass | `uv run pytest -q tests/test_markdown_lane_contract.py tests/test_template_classification.py` enforces Rust/PyO3/Python lane ownership. |
| G5 Test evidence | Targeted tests plus relevant lint/build commands are recorded. | Pass | `uv run pytest -q --ignore=tests/test_quality_gates.py` runs the readiness-focused repository tests. |
| G6 Safety/compliance | Risk, precision, overflow, secrets, async, FFI, and audit checks pass. | Pass | `uv run pytest -q tests/test_dev_guide_sync.py tests/test_v2_guidance_hardening.py tests/test_rust_first_end_to_end.py -k 'safety or fail_closed or precision or overflow or secret or async or ffi or audit or legacy or cython or v1 or advisory'` validates the safety boundaries. |
| G7 Completion report | Findings cite paths, severity, commands, and remaining risk. | Pass | `docs/superpowers/reports/2026-07-29-nt-v2-rust-cutover-reconciliation.md` records findings, commands, gate results, and residual risk. |
