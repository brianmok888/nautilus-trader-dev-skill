...
| G2 Pinned V2 examples | Changed Rust examples compile or have scoped harness evidence. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-review` passed against `c1a2310144c37db80ad11af3d86b65b2ed300c81`; evidence: `references/g2-evidence/nt-review.json`. |
...
- Verify adapter order-status-report filtering follows the shared rule at `crates/live/src/execution/reports.rs` (pinned `c1a2310144c37db80ad11af3d86b65b2ed300c81`, upstream `9b7db823`): open-only venue report requests must retain both open and in-flight reports (`is_open() || is_inflight()`); only the documented Bybit and Polymarket reconciliation exceptions may deviate.
...
Validate review claims against the immutable snapshot under [`references/developer_guide/`](../../references/developer_guide/), particularly `rust.md`, `ffi.md`, `testing.md`, `benchmarking.md`, `adapters.md`, and `coding_standards.md`, pinned to commit `c1a2310144c37db80ad11af3d86b65b2ed300c81`. Version-scope any newer upstream guidance until the pin advances.
