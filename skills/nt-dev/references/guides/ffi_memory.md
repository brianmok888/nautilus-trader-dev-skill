...
| **3** | Foreign                       | Exactly once, call the *type-specific* drop helper exported by Rust (for example `vec_drop_book_levels`, `vec_drop_book_orders`, `vec_drop_fills` (pinned `crates/model/src/ffi/orderbook/{level.rs:120,136}`, `book.rs:457`)). The helper reconstructs the original `Vec<T>` with `Vec::from_raw_parts` and lets it drop, freeing the memory. |
