...
| G2 Pinned V2 examples | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-dev` passed the skill domain's scoped examples and owners against `c1a2310144c37db80ad11af3d86b65b2ed300c81`; schema-v2 provenance is recorded in `references/g2-evidence/nt-dev.json`. |
...
Base developer workflow and FFI claims on [`references/developer_guide/rust.md`](../../references/developer_guide/rust.md) at immutable commit `c1a2310144c37db80ad11af3d86b65b2ed300c81`; label newer upstream behavior as version-scoped drift.
...
### Upstream workspace boundary

For upstream contribution work, create a disposable writable clone or linked
worktree at the requested revision. The pinned cache is evidence-only: never
edit it, build into it, install into it, or prepare commits from it.
...
Environment rules from the upstream contributor contract (upstream `d036f0234`,
root `AGENTS.md` at the pinned baseline):

- No root `.venv`: the uv project environment lives at `python/.venv` (`uv --project python`
  for targeted commands); a virtual environment at the repository root is rejected.
- Run checks locally before proposing changes, report explicitly which checks cannot run,
  and re-run affected checks after edits or rebases — stale evidence does not count.
- `make pre-commit` covers local formatting and lint; `make pre-flight` is the
  higher-assurance local gate and additionally validates Python imports from an isolated
  neutral environment against the already-built package (upstream `b16f1a11f`,
  `scripts/test-python-isolation.bash`), distinct from ordinary pytest in `python/.venv`
  and from CI wheel-isolation validation.
...
- **Dependency features**: trimmed to what each crate consumes — disable unneeded defaults (`default-features = false` for workspace deps), enable required transport and Alloy features at their consumers; enforced by `.pre-commit-hooks/check_dependency_features.py` per-dependency allowlists (upstream `09a235060b`)
- Feature flags: `default = []`, additive, documented at crate level — every non-default feature listed once, alphabetically, under `Feature flags` in both `README.md` and the crate's configured library source (`[lib]` `path` in `Cargo.toml`, default `src/lib.rs`), matching `[features]` in `Cargo.toml`; enforced by `.pre-commit-hooks/check_docs_conventions.sh` (upstream `fd247cda9`)
...
- Upstream `develop` source label is 1.231.0 and `python/pyproject.toml` is
  `2.0.0rc5`; upstream release notes identify 2.0.0rc1 as the first public
...
- Upstream Python v2 controller, execution-algorithm, `FeeModel`, and
  ...
  fee, and fill behavior in Rust. At the pinned develop `c1a2310144c37db80ad11af3d86b65b2ed300c81` (in-pin
