...
generated by the macro. It takes four arguments — `order`, `position_id`, `client_id`, and an
optional `params: Option<Params>` map that execution algorithms and adapters may read
(`crates/trading/src/strategy/mod.rs:172` in the pinned tree; the upstream EMA cross example calls
`self.submit_order(order, None, None, None)`).
