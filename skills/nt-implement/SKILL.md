...
| G2 V2 example validation | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Pending | Fixed-point schema validation and the owning Rust crates passed via `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-implement`, with provenance in `references/g2-evidence/nt-implement.json`; the `capnp` executable is unavailable, so actual schema compilation and round-trip validation remain Pending. A G2 `cargo check` result is compilation only; it is not spec, testnet, resilience, fuzz, or operations acceptance evidence. |
