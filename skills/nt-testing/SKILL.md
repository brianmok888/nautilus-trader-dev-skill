...
`c1a2310144c37db80ad11af3d86b65b2ed300c81` (verified 2026-09-02 via
`tools/upstream_freshness.py`; consult `references/upstream-delta-review.json`
before relying on equality — develop moves independently of this pin). The
...
- The pinned `spec_data_testing.md` documents Rust `subscribe_book_depth(true)`
  support and the current request toggles (`request_book_snapshot`,
...
- adapter precision: replay real payloads with trailing-zero variants and prove field-contract behavior—meaningful venue scale is preserved, documented non-semantic padding is normalized, and instrument or currency precision overrides payload formatting.
- `LiveNodeBuilder`: test same-builder re-entry from a PyO3 factory as an exact recoverable error, active-state `repr`, builder restoration after callback errors, and consuming failures as consumed.
- live reconciliation: cover resolved same-position fill reports, duplicate-report idempotence, side-aware quantity-free close-all restoration, and unresolved in-scope identifiers with non-zero quantities as errors; uncached hedge-mode `PositionStatusReport`s with zero signed quantity reconcile as already-flat (upstream `e98237c91`, `crates/execution/src/engine/mod.rs:1646`).
- execution lifecycle: assert final state, ordering, and absence of duplicate side effects; reject brittle event-count assumptions unless cardinality itself is the public contract.
...
| G2 Pinned V2 examples | Compile or validate examples applicable to this skill against the pinned NT V2 baseline. | Pass | `uv run python tools/check_skill_g2_harnesses.py --execute --skill nt-testing` passed the skill domain's scoped examples and owners against `c1a2310144c37db80ad11af3d86b65b2ed300c81`; schema-v2 provenance is recorded in `references/g2-evidence/nt-testing.json`. |
...
Current-develop overlay (reviewed through `c1a2310144c37db80ad11af3d86b65b2ed300c81`): every public PyO3
...
The previous Python `DataTesterConfig` and `ExecTesterConfig` examples are quarantined under `migration_reference/python/`. Use them only to migrate or compare legacy integrations; Python is not an active production testing lane;

## Source-pinned upstream lane

Use `references/developer_guide/testing.md`, `references/developer_guide/spec_data_testing.md`, `references/developer_guide/spec_exec_testing.md`, and `references/developer_guide/rust.md` as source-pinned upstream snapshots at commit `c1a2310144c37db80ad11af3d86b65b2ed300c81`. Keep the execution-spec freshness and nightly migration notes explicitly version-scoped.
...
- Treat upstream **Python v2 controller subclassing**, **subclassable execution
  ...
  At the pinned develop `c1a2310144c37db80ad11af3d86b65b2ed300c81` (in-pin since `d2b62d35a7`), change
  ...
  same way — reference evidence only.
