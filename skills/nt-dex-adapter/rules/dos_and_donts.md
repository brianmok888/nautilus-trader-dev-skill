...
**DO** use `get_runtime().spawn()` for production adapter tasks; deterministic tests may use `tokio::spawn()` on their own runtime (upstream `6e59fd74eaacacbb7410936f1766bd89fcce6f59` `.pre-commit-hooks/check_tokio_usage.sh` skips adapter test modules).
```rust
...
**DON'T** use `tokio::spawn()` for adapter production tasks; deterministic tests may use it on their own runtime (upstream `6e59fd74eaacacbb7410936f1766bd89fcce6f59` adapter test modules).
```rust
