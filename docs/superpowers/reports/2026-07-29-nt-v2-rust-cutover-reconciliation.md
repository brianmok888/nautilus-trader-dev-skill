...
| Terminal advisory acknowledgments could collide across different records, depend on provenance-FIFO ordering, advance state before exact removal, or be lost during reset. | P0 | Closed | `skills/nt-evomap-integration/templates/advisory_actor.py` derives record-bound SHA-256 checkpoint IDs, separates provenance and terminal outboxes, exposes peek/ack removal, removes the exact checkpoint before floor/authority mutation, and preserves pending acknowledgments across reset; `uv run python tools/run_pinned_v2_pytest.py tests/test_ai_advisory_boundary.py` covers stale same-sequence/outcome acknowledgments, duplicate decisions, occupied provenance, failed removal, and reset recovery. | — |
