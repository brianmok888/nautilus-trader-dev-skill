...
| G2 ownership hashes changed when a long-lived worktree contained empty directories that Git could not reproduce. | P0 | Closed | `tools/g2_owned_content.py` now hashes reproducible file and symlink records rather than ambient directory entries; `tests/test_g2_owned_content.py::test_owned_hash_matches_clean_checkout_with_ambient_empty_directories` compares a source repository containing an untracked empty directory with its clean clone. | — |
...
| Advisory replay protection was bounded by mailbox capacity and reset with actor lifecycle state despite claiming deterministic rejection. | P0 | Closed | `skills/nt-evomap-integration/templates/advisory_actor.py` carries request sequence in terminal audit records, keeps terminal state pending until a durable checkpoint acknowledgment, and rejects sequences at or below restored `request_sequence_floor`; `tests/test_ai_advisory_boundary.py` covers capacity eviction, reset, restart, and a crash before acknowledgment. | — |
| Terminal advisory acknowledgments could collide across different records, depend on provenance-FIFO ordering, advance state before exact removal, be lost during reset, or trust an untested external persistence step. | P0 | Closed | `skills/nt-evomap-integration/templates/advisory_actor.py` derives record-bound SHA-256 checkpoint IDs, separates provenance and terminal outboxes, exposes peek/ack removal, removes the exact checkpoint before floor/authority mutation, and preserves pending acknowledgments across reset; `skills/nt-evomap-integration/python_sidecar/brainstorming_evomap/evomap_capsule_client.py` supplies an atomic SQLite checkpoint store; pinned actor tests plus sidecar fault-injection tests cover stale same-sequence/outcome acknowledgments, duplicate decisions, occupied provenance, failed removal, reset recovery, pre-commit crashes, and lost-ack retries. | — |
...
| Ruff's broad template exclusion allowed the active advisory actor to bypass the full-tree gate. | P1 | Closed | `ruff.toml` excludes only named legacy template trees; `tests/test_quality_gates.py` requires the advisory actor to remain outside the exclusion and lints it explicitly with `--no-force-exclude`. | — |
...
uv run python tools/run_pinned_v2_pytest.py tests/test_ai_advisory_boundary.py
uv run pytest -q skills/nt-evomap-integration/python_sidecar/brainstorming_evomap/tests
