...
| Advisory replay protection was bounded by mailbox capacity and reset with actor lifecycle state despite claiming deterministic rejection. | P0 | Closed | `skills/nt-evomap-integration/templates/advisory_actor.py` rejects request/result sequences at or below a durable `request_sequence_floor`; `tests/test_ai_advisory_boundary.py::test_completed_result_replay_survives_capacity_and_lifecycle_reset` covers capacity eviction, reset, and restart. | — |
