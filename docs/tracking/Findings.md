...
[NT-2026-09-19-001] [P1] [CLOSED 2026-09-19] V2 compliance: upstream develop advanced 178 commits / 1254 paths past the reviewed pin; currency prerequisite requires pin move plus refresh of every pin-citing layer.
  ...
  closure: pin moved to 9bafb63e7d75ab7033aff2e04cd6b4d45d14e9b7 with every pin-citing layer refreshed (22 developer-guide snapshots incl. 3 new pages, CURRENT_SYNC_DATE, curated-mirror citations, taught dependency versions 0.64, inventory/release-lane fixtures, exec-spec digest); all 17 G2 harnesses re-executed at the new pin in the disposable worktree `/tmp/nt-disposable-9bafb63e7` with `check_skill_g2_harnesses.py --check-cards` and `--check-card-declarations` green; `check_upstream_freshness.py --format json` exits 0 with `deltas: []`.
  correction: 2026-09-19 — [P1] — MODIFIED: moved UPSTREAM_COMMIT to 9bafb63e7d75ab7033aff2e04cd6b4d45d14e9b7, refreshed all pin-citing layers, and regenerated all 17 g2-evidence files at the new pin — files: tools/upstream_baseline.py, references/developer_guide/, references/g2-evidence/, skills/*/SKILL.md, docs/tracking/Findings.md

[NT-2026-09-19-002] [P1] [CLOSED 2026-09-19] V2 compliance: mirrored concept/developer-guide/integration snapshots lag the reviewed tip — upstream corrected accounting, networking, reporting, adapter-support, and persistence claims, reorganized architecture and design-policy documentation, added the callback-dispatch and runtime-conformance developer guides, standardized admonition levels, and bumped taught example dependency versions to 0.64.
  ...
  closure: all 22 developer-guide snapshots re-copied from the new pin with refreshed source_commit/sync_date frontmatter (callback_dispatch, python_adapters, runtime_conformance added); curated concept/integration mirrors carry the new pin citation with semantic additions recorded in their own segments; `python3 tools/check_dev_guide_sync.py`, `check_dev_guide_snapshot_sync.py`, and `check_findings_schema.py` all pass.
  correction: 2026-09-19 — [P1] — MODIFIED: re-synced developer-guide snapshot set (22 files incl. 3 new pages) and moved curated-mirror citations to the new pin — files: references/developer_guide/*.md

[NT-2026-09-19-003] [P1] [CLOSED 2026-09-19] Contract drift: upstream now defines a formal callback-dispatch contract and LiveNode queued-callback lifecycle requirements (causal-root preservation through command/event/system/time channels, drain boundaries, ownership clearing during disposal, callback budgets by causal root, bounded dispatch primitives, actor state subscription filters, MessageBus reentry ordering); the repository's contracts teaching predates the entire cluster.
  ...
  closure: design-principles contract extended with callback causal-root preservation and progress-budget semantics; live-runtime contract extended with queued-callback lifecycle (exclusive access, delivery boundaries, disposal ownership clearing); actors concept documents queue/socket state subscription filters; message-bus concept documents reentry ordering and links the pinned callback-dispatch contract.
  correction: 2026-09-19 — [P1] — MODIFIED: extended contracts and concept teaching for the upstream callback-dispatch and queued-callback lifecycle cluster — files: references/developer_guide/contracts/design_principles.md, references/developer_guide/contracts/live_runtime_contract.md, references/concepts/actors.md, references/concepts/message_bus.md

[NT-2026-09-19-004] [P1] [CLOSED 2026-09-19] Stale guidance: upstream renamed the `OrderBookDepth10` family to `OrderBookDepth` throughout Rust and Python (wrangler, subscribe/unsubscribe methods, live commands, data-client hooks, persistence enum spellings, Databento loaders), renamed Tardis depth10 loader/stream functions to depth spellings, and expanded snapshot25 to full 25-level depth; 32+ repository files still teach the removed spellings.
  ...
  closure: all removed depth10-family spellings renamed across nt-data, nt-adapters, nt-model, nt-signals, nt-testing, nt-trading, nt-learn, and the curated concept/integration references (tardis `book_snapshot_output` now teaches `"depth"`; snapshot25 teaches full 25-level snapshots); `tests/test_v2_current_api_spellings.py` guards the active-guidance surface.
  correction: 2026-09-19 — [P1] — MODIFIED: renamed OrderBookDepth10 family to OrderBookDepth across active guidance, retaught tardis depth loaders/streams and the `"depth"` output value — files: skills/nt-data/SKILL.md, skills/nt-data/references/concepts/data.md, skills/nt-data/references/guides/tardis.md, skills/nt-data/references/guides/serialization_patterns.md, skills/nt-adapters/references/integrations/tardis.md, references/integrations/tardis.md, references/concepts/data.md, references/api_reference/model/book.md, skills/nt-model/references/api/model/book.md, tests/test_v2_current_api_spellings.py

[NT-2026-09-19-005] [P1] [CLOSED 2026-09-19] Stale guidance: upstream unified `NautilusDataType` on the model enum — `nautilus_trader.persistence.NautilusDataType` is removed, the `OrderBook` variant and spellings are gone, and `DataBackendSession.add_file` accepts `model.NautilusDataType` rejecting `Instrument`/`Defi`; nt-backtest teaching still uses the old surface.
  ...
  closure: verified current at pin 9bafb63e7d with no repo change required: upstream `docs/how_to/run_rust_backtest.md:152` still imports `NautilusDataType` from the backtest `config` module and the `QuoteTick` variant remains in `crates/model/src/data/mod.rs`; the removed spellings (`persistence` import, `OrderBook` variant) were never taught here. Guarded by `tests/test_v2_current_api_spellings.py::test_nautilusdatatype_teaching_uses_current_variants`.
  correction: 2026-09-19 — [P1] — VERIFIED: NautilusDataType teaching confirmed current at the new pin; no content change needed — files: tests/test_v2_current_api_spellings.py

[NT-2026-09-19-006] [P1] [CLOSED 2026-09-19] Broken example: upstream renamed `TestClock` to `VirtualClock` (117 files); the nt-trading strategy example tests import and construct `TestClock` and no longer compile against the new baseline.
  ...
  closure: `python3 tools/check_rust_trading_reference_sync.py --sync` re-synced the three nt-trading example test files from the new pin (TestClock -> VirtualClock); `python3 tools/check_rust_trading_reference_sync.py` passes and `python3 -m pytest tests/test_rust_first_end_to_end.py -q` passes 8/8 at pin 9bafb63e7d.
  correction: 2026-09-19 — [P1] — MODIFIED: re-synced nt-trading example tests to VirtualClock at the new pin — files: skills/nt-trading/references/examples/rust_trading/examples/strategies/ema_cross/tests.rs, skills/nt-trading/references/examples/rust_trading/examples/strategies/delta_neutral_vol/tests.rs, skills/nt-trading/references/examples/rust_trading/examples/strategies/hurst_vpin_directional/tests.rs

[NT-2026-09-19-007] [P1] [CLOSED 2026-09-19] Contract drift: order and position semantics changed — `OrderCore.events` became read-only via `events()` with `OrderCore::new` construction, avg_px/slippage now use a chronological Decimal fold with rebuild-on-correction, netting reopen cost is independent of replay history, NETTING reductions of external positions fixed, and reversal fills split into closing/opening portions; nt-model/nt-trading order teaching predates these changes.
  ...
  closure: nt-model pins the read-only `events()`/`OrderCore::new` contract and the fixed-point avg_px fold semantics; positions concept documents the reversal closing/opening split and netting reopen-cost independence; no prior repo guidance taught the removed direct field access.
  correction: 2026-09-19 — [P1] — MODIFIED: order accounting and position reversal/reopen teaching refreshed against 21ecc5f854/6df28d559b/7848e5e372 — files: skills/nt-model/SKILL.md, references/concepts/positions.md, skills/nt-trading/SKILL.md

[NT-2026-09-19-008] [P1] [CLOSED 2026-09-19] Stale guidance: upstream standardized adapter config field layouts and URL wiring across 75 files — uniform api_key/api_secret/proxy_url ordering, Databento `historical_base_url`/`live_gateway_addr` overrides, Tardis `tardis_http_url` override, and Hyperliquid Python config parameter order `base_url_http` before `base_url_ws`; integration-guide config tables lag the new layouts.
  ...
  closure: Hyperliquid config tables teach `base_url_http` before `base_url_ws` (Python parameter order) plus `base_url_exchange`; Databento config table adds `historical_base_url` and `live_gateway_addr`; Tardis config tables add `tardis_http_url` — all matching the pinned upstream config layouts.
  correction: 2026-09-19 — [P1] — MODIFIED: standardized adapter config layouts retaught across hyperliquid/databento/tardis mirrors — files: references/integrations/hyperliquid.md, references/integrations/databento.md, references/integrations/tardis.md, skills/nt-data/references/guides/tardis.md, skills/nt-adapters/references/integrations/tardis.md

[NT-2026-09-19-009] [P2] [CLOSED 2026-09-19] Coverage gap: new upstream capabilities absent from repository teaching — Binance RPI orders, Polymarket Data API v2/session keys/Deposit Wallet operations, Lighter 64-bit market IDs and `use_gtd`, sandbox inbound latency modeling, Cap'n Proto instrument schemas, Python custom adapter support, BinaryOption event IDs, bounded actor callback dispatch primitives, and the runtime-conformance/allocator-measurement developer guide.
  ...
  closure: concise pinned-baseline coverage added for Binance RPI orders, Polymarket Data API v2/session keys/Deposit Wallet/book recovery/PolymarketSignatureType, Lighter 64-bit market IDs + use_gtd + book_snapshot_timeout_secs, sandbox inbound latency modeling, Cap'n Proto instrument schemas, and Python custom adapter support; the runtime-conformance guide is mirrored via the developer-guide snapshot set.
  correction: 2026-09-19 — [P2] — MODIFIED: capability coverage added across integration mirrors and skills — files: references/integrations/binance.md, references/integrations/polymarket.md, references/integrations/lighter.md, references/concepts/backtesting.md, skills/nt-implement/SKILL.md, skills/nt-adapters/SKILL.md

[NT-2026-09-19-010] [P2] [CLOSED 2026-09-19] Stale guidance: adapter behavior teaching lags reviewed behavior changes — OKX websocket recovery, order-book recovery/resynchronization, tradeQuoteCcy USD-to-USDC migration, DST wire-test adapter contract; Kraken public-fee fallback and Maker Protection held-order semantics; Betfair SP-bet state, fill, and snapshot reconciliation fixes; Bybit corporate-action execution types; Hyperliquid fail-closed reconciliation snapshots, runtime instrument refresh, quote-denominated quantities, and the inferred-fill commission limitation; live execution mass-status reconciliation semantics.
  ...
  acceptance-test: `python3 tools/check_dev_guide_sync.py` passes; mirrors cite the new pin.
  closure: pinned-baseline behavior notes added for OKX (websocket recovery, tradeQuoteCcy migration, DST wire-test contract) and Hyperliquid (fail-closed reconciliation snapshots, runtime instrument refresh, inferred-fill commission limitation); Kraken/Betfair/Bybit behavior flows through their re-synced mirrors and RELEASES-recorded fixes.
  correction: 2026-09-19 — [P2] — MODIFIED: adapter behavior notes added to OKX and Hyperliquid mirrors — files: references/integrations/okx.md, references/integrations/hyperliquid.md

[NT-2026-09-19-011] [P2] [CLOSED 2026-09-19] Stale guidance: backtest semantics changed — order precision requirements relaxed within the same fixed-point scale, request warnings restored, backtest data-type/missing-engine errors clarified, contingent-order handling improved, and the matching-engine trailing-stop submission policy fixed (in-market trailing stops no longer accepted despite `reject_stop_orders`).
  file: skills/nt-backtest/SKILL.md:244
  ...
  closure: backtesting concept teaches relaxed same-scale precision acceptance, restored data-request warnings, engine/data-type error clarification, trailing-stop `reject_stop_orders` policy, and bidirectional latency modeling at the pinned baseline.
  correction: 2026-09-19 — [P2] — MODIFIED: backtest semantics notes added — files: references/concepts/backtesting.md

[NT-2026-09-19-012] [P2] [CLOSED 2026-09-19] Stale guidance: serialization/catalog surface changed — Parquet catalogs and Arrow encoding refactored (`#4959`), the Arrow display feature renamed to `arrow-display`, catalog re-export and DeFi persistence wiring fixed, and catalog file-extension validation reordered.
  ...
  closure: serialization patterns document the `arrow-display` feature name, pre-rename catalog extension validation, and the Parquet/Arrow refactor pointer with a verify-before-reuse instruction.
  correction: 2026-09-19 — [P2] — MODIFIED: serialization/catalog notes added — files: skills/nt-data/references/guides/serialization_patterns.md
...
[NT-2026-09-09-001] [P1] [CLOSED 2026-09-09] V2 compliance: upstream develop advanced 28 commits / 422 paths past the reviewed pin; currency prerequisite requires pin move plus refresh of every pin-citing layer.
  ...
  acceptance-test: `python3 tools/check_upstream_freshness.py --format json` exits 0; `python3 tools/check_dev_guide_sync.py` passes; `python3 tools/check_rust_trading_reference_sync.py` passes; `python3 tools/check_skill_g2_harnesses.py --check-cards --check-card-declarations` passes; upstream freshness pytest failures clear.
  closure: pin moved to 5e4be2edbf496afcfc5d0aa3a798496fa4493f2f with every pin-citing layer refreshed (snapshots, CURRENT_SYNC_DATE, curriculum pins, README baseline, end-to-end guide, toolchain 1.98.1) and all 17 G2 evidence files regenerated at the new pin.
...
[NT-2026-09-06-001] [P1] [CLOSED 2026-09-06] Legacy unlabelled content: Root Kraken venue guide presents the removed v1 Python adapter config tables as current, unlabelled. Both 'Data client configuration options' and 'Execution client configuration options' tables list v1-only field names (product_types plural, base_url_http_spot, base_url_http_futures, base_url_ws_spot, base_url_ws_futures, base_url_ws_l3_spot, update_instruments_interval_mins, max_retries, retry_delay_initial_ms, retry_delay_max_ms, http_timeout_secs, ws_heartbeat_secs) that exist on neither the pinned Rust configs nor the PyO3 kwargs. A user configuring KrakenDataClientConfig per this table gets unexpected-keyword errors. The sibling skills/nt-adapters copy was rewritten to the pinned struct (yesterday's NT-2026-09-04-215/-228), and the root polymarket.md precedent labels equivalent v1 tables '(legacy v1 Python adapter)'; this file's only label is the line-1 file banner about Cython/v1 TradingNode references, which does not reach a '## Configuration' section read as current guidance.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; fixed in segment F1 (receipt docs/tracking/receipts/harden-nt-v2-20260906/phase-2-seg-f1.json)
...
[NT-2026-09-06-021] [P2] [CLOSED 2026-09-06] Improvement opportunities: Unresolved 'segment S?' placeholder in the closure line of finding NT-2026-09-05-028, which cites skills/nt-review/AGENTS.md (an assigned-group file). The underlying remediation is verifiably complete: the Feature-flags checklist bullet exists at skills/nt-review/AGENTS.md:132 citing upstream fd247cda9, and the phase-2 segment receipt phase-2-seg-s8-trading-review.json explicitly records 'feature-docs checklist item (fd247cda9)'. The closure line was simply never updated from its template placeholder to 'segment S8', unlike the ten sibling entries already pointing at S6. Four further 'S?' placeholders cite files inside this audit group's shared references tree (message_bus.md, developer_guide/adapters.md, integrations/bybit.md, integrations/kraken.md) and are covered by findings NT-2026-09-06-029 through -032; the remaining seven cite other groups' files.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; closed as verified-present: the typed external-streaming coverage the finding gates landed in the shared references tree (payload_kind=typed semantics); F2 verified the artifact exists
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
...
[NT-2026-09-06-022] [P2] [CLOSED 2026-09-06] Improvement opportunities: NT-2026-09-05-028's approved fix required the enforced feature-documentation convention in BOTH nt-review and 'nt-dev Cargo.toml conventions', but only nt-review received it (skills/nt-review/AGENTS.md:132). nt-dev's Rust Conventions section still says only 'Feature flags: `default = []`, additive, documented at crate level' and rust_conventions.md's feature-flag section (lines 877-890) shows a generic module-doc example without the enforced rule: every non-default feature must appear in the alphabetical `Feature flags` lists in both README.md and the crate docs, matching [features] in Cargo.toml, with violations rejected by the pre-commit hook. A developer following nt-dev guidance alone will produce manifests the upstream hook rejects.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; fixed in segment F2 (receipt docs/tracking/receipts/harden-nt-v2-20260906/phase-2-seg-f2.json)
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
...
[NT-2026-09-06-030] [P2] [CLOSED 2026-09-06] Improvement opportunities: False closure for NT-2026-09-05-016 with an unimplementable fix as written: the entry directs implementers to retain_order_status_reports, but its cited file references/developer_guide/adapters.md is a byte-exact source-pinned snapshot of upstream docs/developer_guide/adapters.md at 6df23738 (only front-matter differs), and the upstream doc itself contains zero retain_order_status_reports mentions. Adding the guidance to the snapshot would break check_dev_guide_snapshot_sync; the closure is marked CLOSED with an unresolved 'segment S?' placeholder and no retain_order_status_reports guidance exists anywhere in the skill tree.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; closure corrected 2026-09-06 after phase-5 reconciliation: the original 'verified-present' claim was false - the retain_order_status_reports coverage did not exist; it has now been added to the nt-adapters official spec (implementer contract section) and verified against the pinned reports.rs
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
...
[NT-2026-09-06-041] [P1] [CLOSED 2026-09-06] V2 compliance violations: The MovingAverageFactory guidance documents a v1 enum that does not exist at the pin. The Factory paragraph lists seven SCREAMING_SNAKE variants (`SIMPLE`, `EXPONENTIAL`, `WEIGHTED`, `HULL`, `WILDER`, `DOUBLE_EXPONENTIAL`, `VARIABLE_INDEX_DYNAMIC`) as accepted by the Rust-only `MovingAverageFactory::create`, but at pin 6df23738 `MovingAverageType` has exactly five CamelCase variants (Simple, Exponential, DoubleExponential, Wilder, Hull) and the factory cannot produce a WeightedMovingAverage or VIDYA at all. The companion snippet at lines 283-290 compounds this with Python-styled `ma = MovingAverageFactory.create(20, MovingAverageType.HULL)` using the v1 period-first argument order, contradicting the same paragraph's correct statements that the type comes first and the factory is Rust-only (skills/nt-signals/SKILL.md:99-103 shows the correct call). The section presents itself as current v2 guidance (it cites pinned pyi line numbers), so the whole-file archival banner at the top of the file does not exempt it; that banner should also be narrowed because most of the file is remediated v2 content.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; fixed in segment F3 (receipt docs/tracking/receipts/harden-nt-v2-20260906/phase-2-seg-f3.json)
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
...
[NT-2026-09-06-043] [P1] [CLOSED 2026-09-06] V2 compliance violations: The 'Loading large datasets efficiently' section in the backtesting concept guide teaches a v1 cost model that no longer matches the pinned engine. It claims `BacktestEngine.add_data()` with `sort=True` (the default) re-sorts the entire cumulative stream on every call ('Second call with 1M bars: sorts 2M bars... 1 sort of 10M bars' at line 136) and that deferring to a single `sort_data()` call avoids repeated global sorts. At the pin, `add_data` sorts only the incoming batch, streams are merged by replay timestamp at iteration time, and `sort_data()` does not sort anything - it only sets the `sorted` flag so `run()` stops rejecting the engine. The practical advice (pass `sort=False`, call `sort_data()` before `run()`) is still valid, but the stated bottleneck and the '10M-bar single sort' do not exist at the pin, so performance reasoning based on this section is wrong. The whole-file compatibility note covers retained pre-V2 Python examples, not this current-behavior prose about the v2 engine.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; fixed in segment F3 (receipt docs/tracking/receipts/harden-nt-v2-20260906/phase-2-seg-f3.json)
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
  correction: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; closure extended 2026-09-06 after phase-3 review: the Performance-impact tip box in both copies now teaches the pinned per-batch sort semantics (receipt phase-2-p3-remediation)
...
[NT-2026-09-06-061] [P1] [CLOSED 2026-09-06] V2 compliance violations: The 'Purging cached state' section documents a `purge_from_database=False` parameter on all three Cache purge methods and prose rules keyed on it ('Database deletions occur only when purge_from_database=True'), but the pinned V2 Cache has no such parameter. This is the v1 Cython surface (added to nautilus_trader/cache/cache.pyx by 95835df654, which is in the pin's history but only for the v1 Python implementation); the v2 Rust Cache and its PyO3 stubs expose two-argument methods, so following the documented call raises TypeError.
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; fixed in segment F4 (receipt docs/tracking/receipts/harden-nt-v2-20260906/phase-2-seg-f4.json)
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
  correction: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; closure extended 2026-09-06 after phase-3 review: the two remaining sibling copies (references/concepts/cache.md, nt-data cache.md) now carry the 2-arg pinned surface with the legacy-labelled v1 kwarg note (receipt phase-2-p3-remediation)
...
[NT-2026-09-06-101] [P2] [CLOSED 2026-09-06] Improvement opportunities: option-chain guidance still says 'forward price' where upstream 148e7cf6 renamed the concept to reference pricing for option chain marks
  ...
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; closed after re-attribution: the cited instruments.md never carried the wording; the real site (bybit.md option-chain rows, both copies + spec bootstrap line) was corrected by the mission lead per pinned 148e7cf6
  closure-proof: token residue 0 / coverage present on the fixed tree; python3 -m pytest -q green (451 passed, 7 skipped); all validators green; G2 evidence regenerated PASS at 6df23738
...
[NT-2026-09-05-016] [P2] [CLOSED 2026-09-05] Improvement opportunities: Adapter guidance teaches the open_only/start/end filter contract but not the new shared retain_order_status_reports helper in nautilus-live that standardizes the filtering (open-only includes in-flight; time bounds apply to closed reports only).
  ...
  correction: CORRECTED 2026-09-06 (twice): closure was a placeholder (segment S?) and the improvement never landed; the fix as written is inapplicable to the machine-synced snapshot. Phase-5 reconciliation then caught that the relocation claim was ALSO unlanded; the actual coverage now exists in skills/nt-adapters/references/guides/official_adapter_spec.md (retain_order_status_reports implementer contract, crates/live/src/execution/reports.rs:23 + three call sites)
  closure-proof: token-residue grep on the fixed tree returns 0 stale occurrences; python3 -m pytest -q green on the final tree (452 passed, 7 skipped - the skips are gated v1 modules with explicit migration-reference reasons); check_legacy_labelling + check_findings_schema + G2 check-cards green after final evidence regeneration
  closure: NT v2 compatibility note: legacy tokens quoted above are migration/reference-only audit evidence; see the 2026-09-06 correction note in this entry
